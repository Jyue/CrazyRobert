...
void free_peak_properties(PeakProperties* props) {
    ...
    free_ctrl_buffer(props->peak_heights);
    free_ctrl_buffer(props->prominences);
    free_ctrl_buffer(props->left_bases);
    free_ctrl_buffer(props->right_bases);
    free_ctrl_buffer(props->widths);
    free_ctrl_buffer(props->width_heights);
    free_ctrl_buffer(props->left_ips);
    free_ctrl_buffer(props->right_ips);
    free_ctrl_buffer(props->plateau_sizes);
    free_ctrl_buffer(props->left_edges);
    free_ctrl_buffer(props->right_edges);
...
static size_t _local_maxima_1d(const double* x, size_t n, 
    ...
    bool* is_max = (bool*)get_ctrl_buffer(n * sizeof(bool));
    if (!is_max) {
        return 0; // Memory allocation failed
    }
    // Initialize to zero (equivalent to calloc)
    memset(is_max, 0, n * sizeof(bool));
    ...
    *peaks = (size_t*)get_ctrl_buffer(max_count * sizeof(size_t));
    *left_edges = (size_t*)get_ctrl_buffer(max_count * sizeof(size_t));
    *right_edges = (size_t*)get_ctrl_buffer(max_count * sizeof(size_t));
    
    if (!*peaks || !*left_edges || !*right_edges) {
        free_ctrl_buffer(is_max);
        free_ctrl_buffer(*peaks);
        free_ctrl_buffer(*left_edges);
        free_ctrl_buffer(*right_edges);
        *peaks = NULL;
        *left_edges = NULL;
        *right_edges = NULL;
        return 0; // Memory allocation failed
...
static double scoreatpercentile(const double* data, size_t n, double per) {
    ...
    double* sorted = (double*)get_ctrl_buffer(n * sizeof(double));
...
static bool* _select_by_property_double(const double* property, size_t n,
    ...
    bool* keep = (bool*)get_ctrl_buffer(n * sizeof(bool));
    if (!keep) return NULL;
...
static bool* _select_by_peak_threshold(const double* x, const size_t* peaks, size_t n,
    ...
    *left_thresholds = (double*)get_ctrl_buffer(n * sizeof(double));
    *right_thresholds = (double*)get_ctrl_buffer(n * sizeof(double));
    bool* keep = (bool*)get_ctrl_buffer(n * sizeof(bool));
    
    if (!*left_thresholds || !*right_thresholds || !keep) {
        free_ctrl_buffer(*left_thresholds);
        free_ctrl_buffer(*right_thresholds);
        free_ctrl_buffer(keep);
...
static bool* _select_by_peak_distance(const size_t* peaks, size_t n, const double* values, size_t distance) {
    ...
    size_t* sorted_indices = (size_t*)get_ctrl_buffer(n * sizeof(size_t));
    ...
    bool* keep = (bool*)get_ctrl_buffer(n * sizeof(bool));
    if (!keep) {
        free_ctrl_buffer(sorted_indices);
        return NULL;
    }
...
size_t find_peaks(const double* x, size_t n,
    ...
    *peaks = NULL;
    ...
    size_t* current_peaks = (size_t*)get_ctrl_buffer(peak_count * sizeof(size_t));
    if (!current_peaks) {
        free_ctrl_buffer(all_peaks);
        free_ctrl_buffer(left_edges);
        free_ctrl_buffer(right_edges);
        return 0;
    }
    ...
    if (plateau_size_min != NULL) {
        ...
        size_t* plateau_sizes = (size_t*)get_ctrl_buffer(current_count * sizeof(size_t));
        if (!plateau_sizes) {
            free_ctrl_buffer(all_peaks);
            free_ctrl_buffer(left_edges);
            free_ctrl_buffer(right_edges);
            free_ctrl_buffer(current_peaks);
            return 0;
        }
        ...
        if (!keep) {
            ...
            free_ctrl_buffer(right_edges);
            free_ctrl_buffer(current_peaks);
            free_ctrl_buffer(plateau_sizes);
            return 0;
        }
        ...
        if (current_count > 0) {
            properties->plateau_sizes = (size_t*)get_ctrl_buffer(current_count * sizeof(size_t));
            properties->left_edges = (size_t*)get_ctrl_buffer(current_count * sizeof(size_t));
            properties->right_edges = (size_t*)get_ctrl_buffer(current_count * sizeof(size_t));
    ...
    if (height_min != NULL) {
        // Get peak heights
        double* peak_heights = (double*)get_ctrl_buffer(current_count * sizeof(double));
        if (!peak_heights) {
            free_ctrl_buffer(all_peaks);
            free_ctrl_buffer(left_edges);
            free_ctrl_buffer(right_edges);
        ...
        if (current_count > 0) {
            properties->peak_heights = (double*)get_ctrl_buffer(current_count * sizeof(double));
    ...
    if (distance > 1) {
        ...
        if (properties->peak_heights) {
            ...
            peak_heights = (double*)get_ctrl_buffer(current_count * sizeof(double));
            if (!peak_heights) {
                free_ctrl_buffer(all_peaks);
                free_ctrl_buffer(left_edges);
                free_ctrl_buffer(right_edges);
    ...
    if (prominence_min != NULL || width_min != NULL) {
        ...
        double* prominences = (double*)get_ctrl_buffer(current_count * sizeof(double));
        size_t* left_bases = (size_t*)get_ctrl_buffer(current_count * sizeof(size_t));
        size_t* right_bases = (size_t*)get_ctrl_buffer(current_count * sizeof(size_t));
        ...
        if (!prominences || !left_bases || !right_bases) {
            ...
            free_ctrl_buffer(right_edges);
            free_ctrl_buffer(current_peaks);
            free_ctrl_buffer(prominences);
            free_ctrl_buffer(left_bases);
            free_ctrl_buffer(right_bases);
            free_peak_properties(properties);
            return 0;
        }
        ...
        if (current_count > 0) {
            properties->prominences = (double*)get_ctrl_buffer(current_count * sizeof(double));
            properties->left_bases = (size_t*)get_ctrl_buffer(current_count * sizeof(size_t));
            properties->right_bases = (size_t*)get_ctrl_buffer(current_count * sizeof(size_t));
        ...
        if (width_min != NULL) {
            // Calculate width
            double* widths = (double*)get_ctrl_buffer(current_count * sizeof(double));
            double* width_heights = (double*)get_ctrl_buffer(current_count * sizeof(double));
            double* left_ips = (double*)get_ctrl_buffer(current_count * sizeof(double));
            double* right_ips = (double*)get_ctrl_buffer(current_count * sizeof(double));
            ...
            if (!widths || !width_heights || !left_ips || !right_ips) {
                ...
                free_ctrl_buffer(left_bases);
                free_ctrl_buffer(right_bases);
                free_ctrl_buffer(widths);
                free_ctrl_buffer(width_heights);
                free_ctrl_buffer(left_ips);
                free_ctrl_buffer(right_ips);
                free_peak_properties(properties);
                return 0;
            }
            ...
            if (current_count > 0) {
                properties->widths = (double*)get_ctrl_buffer(current_count * sizeof(double));
                properties->width_heights = (double*)get_ctrl_buffer(current_count * sizeof(double));
                properties->left_ips = (double*)get_ctrl_buffer(current_count * sizeof(double));
                properties->right_ips = (double*)get_ctrl_buffer(current_count * sizeof(double));
    ...
    *peaks = (size_t*)get_ctrl_buffer(current_count * sizeof(size_t));
    if (!*peaks) {
        free_ctrl_buffer(all_peaks);
        free_ctrl_buffer(left_edges);
        free_ctrl_buffer(right_edges);
