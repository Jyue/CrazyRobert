...
PeakResults* initialize_peak_results(int initial_capacity) {
    PeakResults* results = (PeakResults*)get_ctrl_buffer(sizeof(PeakResults));
    if (!results) {
        return NULL;
    }
    ...
    results->indices = (int*)get_ctrl_buffer(initial_capacity * sizeof(int));
...
void free_peak_results(PeakResults* results) {
    ...
    free_ctrl_buffer(results->indices);
    
    if (results->prominences) free_ctrl_buffer(results->prominences);
    if (results->left_bases) free_ctrl_buffer(results->left_bases);
    if (results->right_bases) free_ctrl_buffer(results->right_bases);
    
    if (results->widths) free_ctrl_buffer(results->widths);
    if (results->width_heights) free_ctrl_buffer(results->width_heights);
    if (results->left_ips) free_ctrl_buffer(results->left_ips);
    if (results->right_ips) free_ctrl_buffer(results->right_ips);
    
    if (results->plateau_sizes) free_ctrl_buffer(results->plateau_sizes);
    if (results->left_edges) free_ctrl_buffer(results->left_edges);
    if (results->right_edges) free_ctrl_buffer(results->right_edges);
...
bool expand_peak_results(PeakResults* results) {
    ...
    int* new_indices = (int*)get_ctrl_buffer(new_capacity * sizeof(int));
    ...
    for (int i = 0; i < results->size; i++) {
        new_indices[i] = results->indices[i];
    }
...
void _filter_by_distance(PeakResults* results, const uint32_t* x, int distance) {
    ...
    int* sorted_indices = (int*)get_ctrl_buffer(results->size * sizeof(int));
    int* keep = (int*)get_ctrl_buffer(results->size * sizeof(int));
...
void _calculate_prominences(PeakResults* results, const uint32_t* x, int size, int wlen) {
    ...
    results->prominences = (uint32_t*)get_ctrl_buffer(results->size * sizeof(uint32_t));
    results->left_bases = (int*)get_ctrl_buffer(results->size * sizeof(int));
    results->right_bases = (int*)get_ctrl_buffer(results->size * sizeof(int));
...
void _calculate_widths(PeakResults* results, const uint32_t* x, int size, uint32_t rel_height) {
    ...
    results->widths = (uint32_t*)get_ctrl_buffer(results->size * sizeof(uint32_t));
    results->width_heights = (uint32_t*)get_ctrl_buffer(results->size * sizeof(uint32_t));
    results->left_ips = (uint32_t*)get_ctrl_buffer(results->size * sizeof(uint32_t));
    results->right_ips = (uint32_t*)get_ctrl_buffer(results->size * sizeof(uint32_t));
...
void _calculate_plateau_sizes(PeakResults* results, const uint32_t* x, int size) {
    ...
    results->plateau_sizes = (int*)get_ctrl_buffer(results->size * sizeof(int));
    results->left_edges = (int*)get_ctrl_buffer(results->size * sizeof(int));
    results->right_edges = (int*)get_ctrl_buffer(results->size * sizeof(int));
